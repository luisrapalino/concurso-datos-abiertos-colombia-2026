...
class IngestHealthIndicatorsUseCase:
    """Batch ingestion of curated health indicators from open data sources."""
    ...
    def execute(self, command: IngestHealthIndicatorsCommand) -> IngestionResult:
    ...
    def _apply_territorial_validation(
        self,
        command: IngestHealthIndicatorsCommand,
    ...
    def _fetch_all(
        ...
        collected: list[RawHealthIndicatorRecord] = []
    ...
    def _fetch_for_year(
        ...
        *,
        ...
        while len(collected) < max_records:
            batch = self._source_client.fetch_records(
                year=year,
...
def _resolve_target_years(command: IngestHealthIndicatorsCommand) -> tuple[int | None, ...]:
    if command.years:
