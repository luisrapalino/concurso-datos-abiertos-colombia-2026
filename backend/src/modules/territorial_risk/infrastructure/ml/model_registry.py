...
class ModelRegistry:
    """File-based promotion registry for ML risk model artifacts."""

    MANIFEST_FILENAME = "promoted.json"
    ...
    def get_promotion_state(self) -> PromotionState:
    ...
    def promote(self, version: str) -> PromotionState:
        ...
        if not artifact_path.exists() or not metadata_path.exists():
            msg = f"Model artifacts not found for version {version!r}."
        ...
        payload["history"] = list(payload["history"]) + [
            {
                "action": "promote",
                "version": version,
                "at": payload["promoted_at"],
            },
        ]
        self._write_manifest(payload)
        return self.get_promotion_state()

    def rollback(self) -> PromotionState:
        current = self.get_promotion_state()
    ...
    def artifact_path(self, version: str) -> Path:
        return self._artifacts_dir / f"{version}.joblib"

    def metadata_path(self, version: str) -> Path:
        return self._artifacts_dir / f"{version}.json"
